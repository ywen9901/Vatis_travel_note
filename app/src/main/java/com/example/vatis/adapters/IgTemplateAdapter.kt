...
class IgTemplateAdapter(private val templateItems: ArrayList<TemplateItem>,
                        private val cellClickListener: CellClickListener)
    : RecyclerView.Adapter<IgTemplateAdapter.ViewHolder>(){

    private var selectedPosition = 0 // first template as default

    inner class ViewHolder (itemView: View): RecyclerView.ViewHolder(itemView) {
        fun bindItems(item: TemplateItem) {
            itemView.share_ig_template_item_image.setImageBitmap(item.bitmap)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.ig_template_item, parent, false)
